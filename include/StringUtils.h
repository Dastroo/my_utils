...
template <typename... S> std::string concatenate(S... args) {
  std::stringstream ss;
...
inline std::string
format_param_with_comma(const std::vector<std::pair<T1, T2>> &v) {
  std::stringstream ss;
  ss << std::get<i>(v[0]);
...
inline std::string
format_param_with_comma(const std::vector<std::tuple<Args...>> &v) {
  std::stringstream ss;
  ss << std::get<i>(v[0]);
...
template <size_t i, typename... Args>
inline std::string
format_param_with_comma(const std::vector<std::pair<Args...>> &vector) {
  std::stringstream ss;
...
inline std::string format_with_comma(integer_pack<size_t, indexes...>,
                                     Tuple &&tuple) {
  std::stringstream ss;
...
inline static std::string format_with_comma(Tuple &&tuple) {
  std::stringstream ss;
  ss << std::get<begin>(tuple);
  return ss.str();
}
...
template <typename... Args> inline std::string format_with_comma(Args... args) {
